...
echo "${key}" | "${mcfd}" -r -s -l 127.0.0.1 ${slport} 127.0.0.1 ${scport} > "${dir}/server.out" 2>&1 &
serverpid=${!}

nc6 --idle-timeout=5 -l -p ${clport} < "${dir}/sin" > "${dir}/sout" &
ncserverpid=${!}

sleep 5

"${mcfd}" -r -k "${key}" -l 127.0.0.1 ${clport} 127.0.0.1 ${slport} > "${dir}/client.out" 2>&1 &
clientpid=${!}

nc6 --idle-timeout=5 127.0.0.1 ${scport} < "${dir}/cin" > "${dir}/cout"
